...
from . serializers import SBIcustomerSerializer,SBI_admin_Serializer

#conver_api
class employeeList(APIView):
    def get(self,request):
        customer_data=customer_register_table.objects.all()
        serializer=SBIcustomerSerializer(customer_data,many=True)
        return Response(serializer.data)

class admin_employeeList(APIView):
    def get(self,request):
        customer_data=admin_table.objects.all()
        serializer=SBI_admin_Serializer(customer_data,many=True)
        return Response(serializer.data)
...
def admin_form_submission(request):
    if request.method=='POST':
        if admin_table.objects.filter(admin_username=request.POST.get('admin_username'),admin_password=request.POST.get('admin_password')):
            print("admin login successfully")
            all_users=customer_register_table.objects.all()
            return render(request,'admin_dashboard.html',{'all_users':all_users})
        else:
            print("admin login failed")
            messages.error(request,'invaid admin username or password',extra_tags='failed')
            return render(request,'admin_login.html')
